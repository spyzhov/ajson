...
var (
	...
	Operations = map[string]Operation{
		"**": func(left *Node, right *Node) (result *Node, err error) {
			lnum, rnum, err := CastFloats(left, right)
			if err != nil {
				return
			}
		...
		"/": func(left *Node, right *Node) (result *Node, err error) {
			...
			if rnum == 0 {
				return nil, NewErrorRequest("division by zero")
			}
		...
		"&": func(left *Node, right *Node) (result *Node, err error) {
			lnum, rnum, err := CastInts(left, right)
			if err != nil {
				return
			}
		...
		"+": func(left *Node, right *Node) (result *Node, err error) {
			if left.IsString() {
				lnum, rnum, err := CastStrings(left, right)
				if err != nil {
					return nil, err
				}
		...
		"&&": func(left *Node, right *Node) (result *Node, err error) {
			res := false
			lval, err := CastBoolean(left)
			if err != nil {
				return nil, err
			}
			if lval {
				rval, err := CastBoolean(right)
				if err != nil {
					return nil, err
				}
				res = rval
	...
	Functions = map[string]Function{
		...
		"not": func(node *Node) (result *Node, err error) {
			if value, err := CastBoolean(node); err != nil {
				return nil, err
			} else {
